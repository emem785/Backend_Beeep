...
def mobile_register_civilian(request):
        ...
        if request.method == 'POST':
                ...
                if Civilian.objects.filter(phone = phone).exists() or User.objects.filter(username = phone).exists():
                        ...
                        civilian = Civilian().create(firstname= first_name, lastname= last_name, phone= phone, password= password, email = email, twitter_handle= twitter_handle)
...
def mobile_register_lawyer(request):
        ...
        if request.method == 'POST':
                ...
                if Lawyer.objects.filter(phone = phone).exists() or User.objects.filter(username = phone).exists():
                        ...
                        lawyer = Lawyer().create(firstname= first_name, lastname= last_name, phone= phone, password= password, email = email,twitter_handle=twitter_handle)
